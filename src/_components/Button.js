...
export const Button = styled.button`
  ...
  font-weight: 200;
  float: ${props =>(props.collapse ? 'right' : props.expand ? 'left' : 'none')};
  width: ${props =>(props.collapse || props.expand ? '30px' : '100%')};
  margin: 0;
  margin-top: ${props =>(props.collapse ? '20px' : props.expand ? '16px' : '10px')};
  margin-bottom : ${props =>(props.collapse ? 'auto' : props.expand ? 'auto' : '10px')};
  margin-right: ${props =>(props.collapse || props.expand ? '20px' : '0')};
  padding: ${props =>(props.collapse || props.expand ? '0.4em' : '0.8em')};
  padding-top: ${props => (props.collapse || props.expand ? '0.3em' : props.small ? '0.6em' : '0.9em')};
  padding-bottom: ${props => (props.collapse || props.expand ? '0.3em' : props.small ? '0.55em' : '0.85em')};
  border: 2px solid;
  border-radius: 3px;
